...
function heatmap_plotter_small(data::HDF5Data, slice_index ; savefig = nothing)
    ...
    # Create log-scaled heatmap
    pos = heatmap_ax.pcolor(time , lb_array, intensity_data, 
    ...
    t_realjump = round(tjump_AB - t_0, digits=2)
    # Labels and limits
    #heatmap_ax.set_ylabel(latexstring("\\mathrm{ tjumpAB \\;factor}"))
    heatmap_ax.set_title("d = $d, b= $b, tjump = $t_realjump,  γ = $Gamma, Data: Omega")
    heatmap_ax.set_ylabel(latexstring("\\mathrm{ LB factor \\; [b * b-var] }"))
    heatmap_ax.set_xlabel(latexstring("\\mathrm{Time} \\;t \\; \\; [\\gamma^{-1}]"))
    heatmap_ax.set_xlim(0, 2)
    heatmap_ax.set_ylim(minimum(lb_array), 1)
...
function main()
    ...
    filename = "/Users/denizadiguzel/0.maxplanck/1.anomaly(c)/lb_factor_scan/lb_factor_scan_1c_0.2.h5"
