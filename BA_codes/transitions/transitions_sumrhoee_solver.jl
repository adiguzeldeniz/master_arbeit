...
function multiplesolverandsaver_hdf5()
    params = Dict(
        ...
        :time => zeros(100),
        :runtime => 0.1,
        :slices => [4, 20, 40, 80, 120, 160, 200],
        :exc_array => zeros(20)
    )

    # Define sweep array
    # Coarse grid before and after the fine region
    # coarse_before = collect(0.001:0.05:0.979)
    # fine = collect(0.980:0.005:1.080)
    # coarse_after = collect(1.081:0.05:2.100)

    # exc_array = vcat(coarse_before, fine, coarse_after)

    # Define sweep array
    par_start, par_end, par_step = 0.001, 4.101, 0.001
    exc_array = collect(par_start:par_step:par_end)
    num_sweeps = length(exc_array)

    # Run one sample solution to determine time steps
    sample_sol = EOM_double_solver(params)
    num_time_steps = size(sample_sol[1], 2)
    N = params[:N]

    file_name = "transitions_paper_full_4pi.h5"
    h5open(file_name, "w") do file
        # Save the full sweep array
        file["exc_array"] = exc_array
        ...
        for (key, value) in params
            params_group[string(key)] = value
        end

        # Create groups for each field and selected slices
        ...
        # Create group for sum_rhoee
        sum_group = create_group(file, "sum_rhoee")
        for slice_index in params[:slices]
            create_dataset(sum_group, string(slice_index), Float64, num_sweeps, num_time_steps)
        end

        for (j, exc_fact) in enumerate(exc_array)
            println("Solving for exc factor: $exc_fact ($j / $num_sweeps)")
            params[:exc] = exc_fact

            sol = EOM_double_solver(params)

            rho_ee = sol[3]  # (N, num_time_steps)
            ...
            for slice_index in params[:slices]
                ...
                # Save sum_rhoee up to slice_index
                sum_over_slices = sum(rho_ee[1:slice_index, :], dims=1)
                file["sum_rhoee/$(slice_index)"][j, :] = vec(sum_over_slices)
            end

            # Save full solution for selected excitation values
            if exc_fact in (0.02, 0.1, 0.4, 0.8, 0.95, 1.1, 1.5, 1.9, 2, 2.1, 3, 4)
                exc_group = create_group(file, "full_slices/exc_$(exc_fact)")

                for (i, group_name) in enumerate(["Omega", "rho_eg", "rho_ee", "rho_gg"])
                    g = create_group(exc_group, group_name)
                    data = sol[i]  # (N, num_time_steps), complex
                    g["data"] = data
                end
            end
